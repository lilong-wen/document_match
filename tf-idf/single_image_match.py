...
def read_data(path):
    ...
    for item in result_dict.items():
        contents.append((str(item[0]), " ".join(item[1])))
...
def ocr_content_imgs(imgs_path, json_path):

    print("ocr every scanned images")

    reader = ocr_engine.Reader()
    scanned_imgs = glob.glob(imgs_path + "*")

    scanned_result_dict = {}
    for scanned_item in scanned_imgs:
        print(scanned_item)
        item_result = reader.readtext(scanned_item)
        scanned_result_dict[str(scanned_item.split("/")[-1][:-4])] = item_result

    with codecs.open(json_path, 'w', encoding='utf-8') as scanned_f:
...
if __name__ == '__main__':
    ...
    parser.add_argument("--query_img", required=True, help='query image')
    args = parser.parse_args()

    source_images_path = "./data/scanned/"
    json_path = './data/scanned_ocr_result.json'

    if os.path.exists(json_path):
        print("ocr for scanned images already done")
    else:
        ocr_content_imgs(source_images_path, json_path)

    reader = ocr_engine.Reader()
    query_text = " ".join(reader.readtext(args.query_img))
    # query_dict (key: str)
    # modify this if query key is not the image name
    query_content = [(args.query_img.split("/")[-1][:-4], query_text)]
    # print(query_content)
    queries = pre_process(query_content)
    ...
    if os.path.exists("pre_processed_data.json"):
        pre_processed_data = read_dict("pre_processed_data.json")
    else:
        pre_processed_data = pre_process(read_data(json_path))
        save_dict(pre_processed_data, "pre_processed_data")
    ...
    query_dict_corpus = pre_processed_data.copy()
    query_dict_corpus.update(queries)
